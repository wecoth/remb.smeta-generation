...
export function worldUnitsFromPixels(px) { return Math.max(0, px / Math.max(_scale, 0.0001)); }
...
export function snap(x, y, options = {}) {
  ...
  if (!forceNoEndpoint) {
    const snapDist = worldUnitsFromPixels(options.endpointFallbackTolerancePx ?? 20);
...
function distanceToSegmentScreen(screenPoint, a, b) {
  const dx = b.x - a.x;
  const dy = b.y - a.y;
  const len2 = dx * dx + dy * dy;
  if (len2 < 1e-6) return Math.hypot(screenPoint.x - a.x, screenPoint.y - a.y);
  let t = ((screenPoint.x - a.x) * dx + (screenPoint.y - a.y) * dy) / len2;
  t = Math.max(0, Math.min(1, t));
  const px = a.x + dx * t;
  const py = a.y + dy * t;
  return Math.hypot(screenPoint.x - px, screenPoint.y - py);
}
...
export function findGuideCandidate(screenPoint) {
  ...
  for (const wall of appState.walls) {
    for (const corner of getWallGuideCorners(wall)) {
      ...
      const distance = Math.hypot(screenPoint.x - screen.x, screenPoint.y - screen.y);
      tryRegister({ id: corner.id, wallId: wall.id, anchor: corner.point, dir: corner.dir, distance });
    }
  }
  for (const jr of getWallJointRects()) {
    for (const point of getJointLocalCornerPoints(jr)) {
      const screen = toScreen(point.x, point.y);
      const distance = Math.hypot(screenPoint.x - screen.x, screenPoint.y - screen.y);
      tryRegister({ id: `joint:${jr.key}`, wallIds: jr.wallIds, anchor: point, dir: { x: 1, y: 0 }, distance });
    }
  }
  ...
  for (const wall of appState.walls) {
    ...
    if (len < 1) continue;
    const proj = projectPointOntoSegment(worldPoint, seg);
    const ds = toScreen(seg.x1, seg.y1);
    const de = toScreen(seg.x2, seg.y2);
    const dist = distanceToSegmentScreen(screenPoint, ds, de);
    if (dist <= lineTolerance) {
      tryRegister({
        id: `wall-line:${wall.id}:${Math.round(proj.x)},${Math.round(proj.y)}`,
        wallId: wall.id,
        anchor: { x: proj.x, y: proj.y },
        dir: { x: (seg.x2 - seg.x1) / len, y: (seg.y2 - seg.y1) / len },
        distance: dist,
      });
    }
  }

  // Поддержка направляющих и для рулетки/зоны: конечные точки этих линий
  // тоже могут выступать якорями "рельсы".
  if (appState.dividers?.length) {
    for (const d of appState.dividers) {
      const dx = d.x2 - d.x1, dy = d.y2 - d.y1;
      const len = Math.hypot(dx, dy);
      if (len < 1) continue;
      const dir = { x: dx / len, y: dy / len };
      for (const p of [{ x: d.x1, y: d.y1 }, { x: d.x2, y: d.y2 }]) {
        const s = toScreen(p.x, p.y);
        const distance = Math.hypot(screenPoint.x - s.x, screenPoint.y - s.y);
        tryRegister({ id: `divider:${d.id}:${Math.round(p.x)},${Math.round(p.y)}`, dividerId: d.id, anchor: p, dir, distance });
      }
      const seg = { x1: d.x1, y1: d.y1, x2: d.x2, y2: d.y2 };
      const proj = projectPointOntoSegment(worldPoint, seg);
      const s1 = toScreen(d.x1, d.y1);
      const s2 = toScreen(d.x2, d.y2);
      const dist = distanceToSegmentScreen(screenPoint, s1, s2);
      if (dist <= lineTolerance) {
        tryRegister({
          id: `divider-line:${d.id}:${Math.round(proj.x)},${Math.round(proj.y)}`,
          dividerId: d.id,
          anchor: { x: proj.x, y: proj.y },
          dir,
          distance: dist,
  ...
  if (appState.measures?.length) {
    for (const m of appState.measures) {
      ...
      for (const p of [{ x: m.x1, y: m.y1 }, { x: m.x2, y: m.y2 }]) {
        const s = toScreen(p.x, p.y);
        const distance = Math.hypot(screenPoint.x - s.x, screenPoint.y - s.y);
        tryRegister({ id: `measure:${m.id}:${Math.round(p.x)},${Math.round(p.y)}`, measureId: m.id, anchor: p, dir, distance });
      }
      const seg = { x1: m.x1, y1: m.y1, x2: m.x2, y2: m.y2 };
      const proj = projectPointOntoSegment(worldPoint, seg);
      const s1 = toScreen(m.x1, m.y1);
      const s2 = toScreen(m.x2, m.y2);
      const dist = distanceToSegmentScreen(screenPoint, s1, s2);
      if (dist <= lineTolerance) {
        tryRegister({
          id: `measure-line:${m.id}:${Math.round(proj.x)},${Math.round(proj.y)}`,
          measureId: m.id,
          anchor: { x: proj.x, y: proj.y },
          dir,
          distance: dist,
...
export function shouldKeepGuideLine(screenPoint, guide, guideReleasePx = 48, anchorReleasePx = 64) {
  ...
  const anchorScreen = toScreen(guide.anchor.x, guide.anchor.y);
  const anchorDistance = Math.hypot(screenPoint.x - anchorScreen.x, screenPoint.y - anchorScreen.y);
  return guideDistance <= guideReleasePx || anchorDistance <= anchorReleasePx;
}

export function getNearestGuideLineAxis(screenPoint, guide) {
  const nearest = getNearestGuideAxis(screenPoint, guide);
  return nearest ? { anchor: guide.anchor, dir: nearest.dir } : guide;
}
