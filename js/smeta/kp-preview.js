...
async function generateRoadmapText() {
  ...
  // ── Промт ─────────────────────────────────────────────────────
  const prompt = `Тебе переданы этапы ремонта квартиры и список работ внутри них.
Напиши деловое описание хода ремонта для коммерческого предложения.
...
ЭТАПЫ РЕМОНТА
(Порядок строк отражает реальную последовательность работ)
 
${stagesContext}
 
ЭТАЛОННЫЙ ПРИМЕР ТОНА И СТИЛЯ
Именно так должен звучать результат — ориентируйся на этот фрагмент:
...
ПРАВИЛА
— Пиши в точно таком же деловом тоне — сухо, конкретно, без лирики
— Охвати все этапы из сметы, группируя родственные работы в одну мысль
— Используй строительные термины как есть, без объяснений в скобках
— 4–6 абзацев, каждый 2–4 предложения
— Без заголовков, без списков, без нумерации
— Текст начинается сразу с первого предложения по существу
 
ЗАПРЕЩЕНО
Слова: высокое качество, премиальные материалы, наша команда, профессиональное выполнение, идеальный результат, надёжный, современный, под ключ, гарантируем.
Восклицательные знаки. Цены и сроки цифрами.
