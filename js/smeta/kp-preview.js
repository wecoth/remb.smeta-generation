...
async function generateRoadmapText() {
  ...
  // ── Контекст объекта (без изменений) ─────────────────────────
  const smrRows   = appState.smrRows  || [];
  ...
  const objectContext = [
    ...
    totalDays          ? `Плановый срок: ${totalDays} рабочих дней`        : null,
  ].filter(Boolean).join('. ');
 
  // ── Сигналы качества (без изменений) ─────────────────────────
  ...
  // ── Этапы с работами — структурированный вход ────────────────
  // Фильтруем технический мусор (вывоз, мусор) — он не нужен клиенту
  const stagesForAnalysis = stages
  ...
  // ── ШАГ 1: аналитический запрос ──────────────────────────────
  // GPT читает работы и возвращает JSON с тем, что важно объяснить клиенту.
  // Маленький чёткий запрос — маленький чёткий ответ.
 
  if (statusEl) statusEl.textContent = 'Анализирую смету...';
 
  const analysisPrompt = `Ты — аналитик строительных смет. Твоя задача — не писать текст, а извлечь из сметы то, что важно объяснить клиенту-неспециалисту.
 
ДАННЫЕ ОБЪЕКТА
${objectContext || 'не указаны'}
 
ЭТАПЫ РАБОТ
${stagesForAnalysis.map(s =>
  `Этап ${s.index} — "${s.name}":\n${s.works.map(w => `  - ${w}`).join('\n')}`
).join('\n\n')}
 
ЗАДАЧА
Для каждого этапа определи:
1. "summary" — 1-2 предложения: что происходит на этом этапе простыми словами (без строительных терминов)
2. "keyWorks" — 2-3 самые важные работы из списка, которые стоит упомянуть клиенту (строки из сметы, дословно)
3. "whyItMatters" — 1 предложение: почему этот этап важен или что будет если его сделать плохо (если применимо)
4. "clientResult" — 1 предложение: что клиент увидит или почувствует по окончании этапа
5. "isCritical" — true если этап содержит скрытые работы (коммуникации, гидроизоляция, основание), false иначе
 
Также определи:
- "qualityHighlights" — массив из технических решений которые выгодно отличают этот ремонт: ${qualitySignals || 'нет особых решений'}
- "stagesCount" — общее количество этапов в смете
 
Верни ТОЛЬКО валидный JSON без комментариев, без markdown, без пояснений. Структура:
{
  "stages": [
    {
      "index": 1,
  ...
  let analysis;
  try {
    const res1 = await fetch(GAS_PROXY_URL, {
      method:   'POST',
      redirect: 'follow',
    ...
    if (!raw1 || raw1.startsWith('<')) throw new Error('Неверный ответ от прокси на шаге 1');
 
    // Чистим от возможных markdown-кавычек
    const cleaned = raw1.trim().replace(/^```json\s*/i, '').replace(/^```\s*/i, '').replace(/```\s*$/i, '').trim();
    analysis = JSON.parse(cleaned);
 
  } catch (e) {
    console.error('[Roadmap] Ошибка шага 1 (анализ):', e);
    if (statusEl) statusEl.textContent = '✗ Ошибка анализа: ' + e.message;
    if (btnEl) btnEl.disabled = false;
    return;
  }
 
  // ── ШАГ 2: нарративный запрос ────────────────────────────────
  // GPT получает чистый JSON с акцентами → пишет живой текст.
  // Вход структурирован — выход предсказуем.
 
  if (statusEl) statusEl.textContent = 'Пишу текст...';
 
  // Формируем компактный контекст из результатов анализа
  const analysisContext = analysis.stages.map(s => {
    const lines = [
      `ЭТАП ${s.index}: ${s.name}`,
      `Суть: ${s.summary}`,
      `Ключевые работы: ${s.keyWorks.join(', ')}`,
      s.whyItMatters ? `Важность: ${s.whyItMatters}` : null,
      `Результат для клиента: ${s.clientResult}`,
      s.isCritical ? `[СКРЫТЫЙ ЭТАП — нужен акцент на необратимости]` : null,
    ].filter(Boolean).join('\n');
    return lines;
  }).join('\n\n');
 
  const highlightsLine = analysis.qualityHighlights && analysis.qualityHighlights.length
    ? `Особые технические решения в этом проекте: ${analysis.qualityHighlights.join(', ')}.`
    : '';
 
  const narrativePrompt = `Ты — прораб, который спокойно и по-человечески объясняет заказчику-неспециалисту что и зачем будет происходить у него в квартире. Без рекламы, без пафоса.
 
АНАЛИТИКА ПО ЭТАПАМ (уже обработана, используй её как основу):
...
ФОРМАТ ВЫВОДА
${analysis.stagesCount <= 3
  ? '3-4 ёмких абзаца свободной прозой. Каждый абзац 3-5 предложений.'
  : '4-5 ёмких абзацев свободной прозой. Каждый абзац 3-5 предложений. Родственные этапы можно объединять в один абзац.'}
Без заголовков, без меток типа "Этап 1:" или "Подготовка:", без буллетов, без списков.
Текст начинается сразу с первого предложения по существу — без вступлений и разогрева.
Между абзацами — пустая строка.
 
ПЕРЕХОДЫ
Не начинай каждый абзац с "Далее" / "После этого" — варьируй переходы через действие или причину:
"Когда черновое готово...", "Пока стены сохнут...", "Параллельно идёт...", "К моменту чистовой...", "Как только...", "На этом же этапе..."
 
ПЕРВОЕ ПРЕДЛОЖЕНИЕ
Начни с действия и его смысла. Не с "Перед началом работ" и не с названия этапа.
Пример: "До того как начнётся шум и пыль, защищаем то, что должно остаться целым — и подводим временный свет и воду, без которых дальше никак."
 
ЯЗЫК
Технические термины — только с пояснением в скобках или заменой бытовым словом:
"штробление" → "прорезаем канавки в стенах под провода"
"маячковый профиль" → "металлические направляющие, по которым ровняется штукатурка"
"инсталляция" → "скрытая рама в стене, на которую подвешивается унитаз"
"стяжка" → "выравнивающий слой пола"
"гидроизоляция" → можно оставить, она понятна
 
ДЛЯ СКРЫТЫХ ЭТАПОВ (помечены [СКРЫТЫЙ ЭТАП])
Добавь 1 предложение о необратимости — не лозунгом, а через причину:
"переделать это после укладки плитки невозможно — поэтому проверяем дважды"
"если здесь схалтурить, через год потечёт к соседям"
 
ЗАПРЕТЫ
Запрещены слова: качественно, профессионально, надёжный, современный, под ключ, наша команда, наши специалисты, опытные мастера, лучшие материалы, идеальный, премиум, гарантируем, обеспечивает, осуществляется, производится, выполняется (как пустой глагол).
...
Запрещено перечислять работы списком длиннее 3-4 пунктов подряд — это превращает текст в смету.
 
Верни только готовый текст. Ничего больше.`;
 
  try {
    const res2 = await fetch(GAS_PROXY_URL, {
      method:   'POST',
      redirect: 'follow',
      body:     JSON.stringify({ prompt: narrativePrompt }),
    });
 
    const raw2 = await res2.text();
    if (!raw2 || raw2.startsWith('<')) throw new Error('Неверный ответ от прокси на шаге 2');
 
    _roadmapNarrative = raw2.trim();
    fillRoadmap();
    ...
    console.error('[Roadmap] Ошибка шага 2 (нарратив):', e);
    if (statusEl) statusEl.textContent = '✗ Ошибка генерации: ' + e.message;
  }
