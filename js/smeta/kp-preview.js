...
async function generateRoadmapText() {
  ...
  const objectContext = [
    ...
    (doors || windows) ? `Проёмов: дверных ${doors}, оконных ${windows}`     : null,
  ...
  // ── Этапы: только названия + ключевые работы (без мусора) ────
  // Берём не все строки, а только топ-3 значимых работы на этап —
  // чтобы не раздувать вход и не провоцировать GPT на перечисление
  const stagesContext = stages
    ...
    .map((s, i) => {
      const works = s.smrRows
        ...
        .slice(0, 4) // максимум 4 работы на этап — остальное GPT обобщит сам
        .map(r => r.name.trim())
        .filter(Boolean);
      return `${i + 1}. ${s.name}: ${works.join(', ')}`;
    })
    .join('\n');
 
  // ── Промт ─────────────────────────────────────────────────────
  const prompt = `Ты — прораб, объясняешь заказчику-неспециалисту что будет происходить в его квартире. Спокойно, по-человечески, без рекламы.
 
ОБЪЕКТ: ${objectContext || 'данные не указаны'}
${qualitySignals ? `ОСОБЫЕ РЕШЕНИЯ: ${qualitySignals}` : ''}
 
ЭТАПЫ РАБОТ:
${stagesContext}
 
ЗАДАЧА
Напиши связный текст — обзор всего хода ремонта для клиента. Охвати все этапы, но не пересказывай смету. Объясняй зачем каждый этап нужен и что клиент получит на выходе.
 
ЖЁСТКИЕ ТРЕБОВАНИЯ К ОБЪЁМУ
Ровно 3 абзаца. Каждый абзац — 2-3 предложения. Итого не более 170 слов. Это не рекомендация — это ограничение под размер блока на странице.
 
ФОРМАТ
Свободная проза. Без заголовков, без меток этапов, без буллетов. Первое предложение — сразу с действия. Между абзацами пустая строка.
 
ПЕРЕХОДЫ
Не начинай каждый абзац одинаково. Варьируй: "Когда черновое готово...", "Параллельно идёт...", "К финишу...", "Как только...", "На этом же этапе..."
 
ЯЗЫК
Термины только с пояснением: "штробление" = "канавки в стенах под провода", "стяжка" = "выравнивающий слой пола", "инсталляция" = "скрытая рама под унитаз". Если этап содержит скрытые работы (коммуникации, гидроизоляция) — одно предложение про необратимость: "переделать это после плитки невозможно".
 
ЗАПРЕЩЕНО
Слова: качественно, профессионально, надёжный, современный, под ключ, наша команда, специалисты, гарантируем, обеспечивает, осуществляется, производится.
Восклицательные знаки. Цены, сроки цифрами, метражи.
Перечисления длиннее 3 пунктов через запятую.
 
Верни только текст. Ничего больше.`;
  ...
  try {
    const res = await fetch(GAS_PROXY_URL, {
      ...
      body:     JSON.stringify({
        prompt,
        maxTokens: 700,  // GPT получает лимит — 170 слов ≈ 600-650 токенов, берём с запасом
      }),
    });
