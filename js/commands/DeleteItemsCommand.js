...
export class DeleteItemsCommand extends BaseCommand {
  constructor(items) {
    ...
    const roomIds = new Set(items.filter(i => i.type === 'room').map(i => i.id));

    this._walls = appState.walls.filter(w => wallIds.has(w.id)).map(w => ({ ...w }));
    this._openings = appState.openings.filter(
      o => openIds.has(o.id) || wallIds.has(o.wallId)
    ).map(o => ({ ...o }));
    ...
    this._rooms = appState.rooms?.filter(r => roomIds.has(r.key)).map(r => ({ ...r })) || [];

    const counts = [
      `${this._walls.length} ст.`,
      `${this._openings.length} пр.`,
      `${this._measures.length} изм.`,
      `${this._dividers.length} зон`,
      `${this._rooms.length} комн.`,
    ].filter(s => !s.startsWith('0 ')).join(', ');
    this.description = `Удаление: ${counts || 'ничего'}`;
  }
  ...
  undo() {
    ...
    if (appState.rooms)    appState.rooms.push(...this._rooms.map(r => ({ ...r })));
