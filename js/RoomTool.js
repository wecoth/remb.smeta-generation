...
export class RoomTool extends BaseTool {
  constructor(ui) {
    super(ui);
    this.name = 'room';
    this.hoverPolygon = null;       // подсвеченный полигон
  ...
  onMouseMove(pos, world, e) {
    // Находим все замкнутые контуры (faces), как это делает комната
    ...
    const allWalls = [...walls]; // разделители тоже добавляются, если есть
    ...
    // Ищем подходящий фейс, внутри которого находится курсор
    let best = null;
    for (const face of faces) {
      const poly = face.map(v => ({ x: v.x, y: v.y }));
      if (poly.length < 3) continue;
      if (polygonArea(poly) < 50000) continue; // меньше 0.05 м² – игнорируем
      if (isPointInPolygon(world, poly)) {
        // Проверяем, не находится ли точка в другой комнате (чтобы не создавать дубли)
