...
function drawRoomFills(selectedItems) {
  ...
  for (let i = 0; i < appState.rooms.length; i++) {
    ...
    if (!r.cells?.length && !r.polygon) continue;
    _ctx.save();

    if (r.polygon) {
      // ── Векторный режим: комната задана полигоном (voids из polygon-clipping) ──
      _ctx.beginPath();
      ...
      // ── Растровый fallback (raster flood fill) ──────────────────
      const OVERLAP_MM = 32;
      _ctx.beginPath();
...
function drawWalls(selectedItems) {
  ...
  // ── Pass 1: единый монолитный fill ─────────────────────────────
  // Все стены объединены в один полигон — никаких внутренних швов.
  // Заливка идёт одним путём, хэтчинг тоже.
  if (unified.length > 0) {
    fillWall(() => {
      _ctx.beginPath();
  ...
  // ── Pass 3: выделение выбранных стен поверх ─────────────────
  // Рисуем индивидуальный прямоугольник выбранной стены поверх union.
