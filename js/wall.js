...
function findReusableWall(start, end, thick) {
  ...
  for (const w of appState.walls) {
    ...
    // с первой.
    //
    // НОВАЯ МОДЕЛЬ (Renga-style): стена остаётся на своём месте без
    // изменений. Существующая стена обслуживает обе комнаты:
    //   - её cx/cy = внутренняя грань первой комнаты
    //   - её противоположная грань (cx ± thickness по нормали) = внутренняя
    //     грань второй комнаты
    //
    // В графе помещений (geometry.js) каждая стена даёт ДВА ребра — по
    // обеим граням. Поэтому контуры обеих комнат корректно замыкаются
    // даже без четвёртой "своей" стены у второй комнаты.
